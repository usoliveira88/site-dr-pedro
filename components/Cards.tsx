...
export function PatientNeedCard({ text, index }: { text: string; index: number }) {
  return (
    <div className="hover-ink-card group rounded-subtle border border-deep/10 bg-white p-6 shadow-[0_10px_32px_rgba(2,37,61,0.07)] transition duration-300 hover:-translate-y-1 hover:shadow-lift">
...
export function InfoCard({ title, text }: { title: string; text: string }) {
  return (
    <div className="hover-ink-card group rounded-subtle border border-deep/10 bg-white p-6 shadow-[0_10px_32px_rgba(2,37,61,0.07)] transition duration-300 hover:-translate-y-1 hover:shadow-lift">
      ...
      <p className="hover-ink-muted mt-3 text-sm leading-7 text-graphite">{text}</p>
    </div>
  );
}
